...
// классы позиции/размера подобраны по side здесь, в JS, а не через
// `data-[side=right]:w-3/4` в className: у Tailwind такой вариант компилируется
// в селектор класс+атрибут (специфичность 0,2,0), который всегда перебивает
// простой utility-класс вроде `w-full`, переданный потребителем через cn() —
// поэтому переопределить ширину/высоту снаружи было физически невозможно
